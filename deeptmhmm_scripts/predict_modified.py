...
if os.environ.get('BIOLIB_JOB_MAX_RUNTIME_IN_SECONDS'):
    overhead_in_seconds = 20
    throughput_in_bytes_per_second = 700
    fasta_size = os.stat(args.fasta).st_size
    estimated_running_time = overhead_in_seconds + (fasta_size / throughput_in_bytes_per_second) * 1.2

    if estimated_running_time > int(os.environ['BIOLIB_JOB_MAX_RUNTIME_IN_SECONDS']):
        #open('/deeptmhmm_results.md', 'w').write(f"The input file ({fasta_size} bytes) is too large for the app be able to finish within the allowed max runtime.")
        sys.stderr.write(f"The input file ({fasta_size} bytes) is too large for the app be able to finish within the allowed max runtime.")
        sys.exit(27)
...
with open("predicted_topologies.3line", "w") as outfile:
    outfile.write(predicted_topologies)

markdown_outfile = open('deeptmhmm_results.md' , 'w')
...
with open("TMRs.gff3", "w") as outfile:
...
if not len(protein_sequences) == 1:
    ...
    print('![picture](plot.png)', file=markdown_outfile)

    write_probabilities_to_file(seq, seq_id, marg_prob_plot)
